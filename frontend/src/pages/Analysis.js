...
function Analysis() {
  ...
  const getHarmonyResult = (behavior) => {
      return {
        result: "🟢 긍정적인 관계 행동이 관찰됨",
        detail: `
    선택한 행동인 "${behavior}"은 두 반려동물 사이에서 긍정적인 상호작용이 나타나고 있음을 의미합니다.

    함께 놀이하거나 가까이 쉬는 행동은 서로에 대한 경계심이 낮아지고,
    같은 공간을 비교적 안정적으로 받아들이고 있다는 신호일 수 있습니다.

    다만 흥분도가 지나치게 높아지면 놀이가 다툼으로 바뀔 수 있으므로
    보호자가 놀이 강도와 시간을 관찰하는 것이 좋습니다.
        `,
        recommendation: [
          "긍정 행동이 나타날 때 칭찬과 보상 제공하기",
          "놀이 시간이 과열되기 전에 짧게 쉬게 하기",
          "각자의 휴식 공간은 따로 유지하기",
          "좋은 상호작용을 짧고 자주 반복하기",
        ],
      };
...
은신처를 충분히 제공해야 합니다.
  ...
  const runAnalysis = async () => {
    ...
    const finalBehavior = customBehavior.trim()
  ? customBehavior
  : selectedBehavior;

    const behaviorInfo = getBehaviorInfo(finalBehavior);

    const keywordResult = customBehavior.trim()
      ? findKeywordSolution(customBehavior)
      : null;

    let result = keywordResult || analyzePets(p1, p2);

    // 선택 행동이 화합이면 결과를 화합 내용으로 강제 보정
    if (behaviorInfo.trend === "harmony") {
      result = getHarmonyResult(finalBehavior);
    }

    // 선택 행동이 스트레스면 결과를 스트레스 내용으로 보정
    if (behaviorInfo.trend === "stress") {
      result = getStressResult(finalBehavior);
    }
